...
export const useGetAnalyticsReach = ({ channel_id, date_from, date_to }) => {
    const { data: analyticsReach, isPending: analyticsReachPending } = useQuery({
        queryKey: ["analyticsReach", channel_id, date_from, date_to],
        queryFn: () => getAnalyticsReach({ channel_id, date_from, date_to }),
        enabled: !!channel_id,
    });
