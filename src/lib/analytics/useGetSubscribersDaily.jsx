...
export const useGetSubscribersDaily= ({ channel_id, date_from, date_to }) => {
    const { data: subscribersDaily, isPending: subscribersDailyPending } = useQuery({
        queryKey: ["subscribers-daily", channel_id, date_from, date_to],
        queryFn: () => getSubscribersDaily({ channel_id, date_from, date_to}),
        enabled: !!channel_id,
    });
